...
/**
 * Controller tests using standalone MockMvc setup with constructor injection. These tests focus on
 * validating the REST API contract, HTTP status codes, and proper request/response handling without
...
public class BookControllerTest {
  ...
  void getBookByIsbn_WithValidIsbn_ShouldReturnBook() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/books/isbn/1234567890"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.id", is(1)))
        .andExpect(jsonPath("$.title", is("Test Title")))
  ...
  void getBookByIsbn_WithInvalidIsbn_ShouldReturnNotFound() throws Exception {
    ...
    when(bookService.getBookByIsbn("invalid"))
        .thenThrow(new BookNotFoundException("Book not found with ISBN: invalid"));

    // Act & Assert
    mockMvc
        .perform(get("/api/books/isbn/invalid"))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.status", is(404)))
        .andExpect(jsonPath("$.error", is("Not Found")))
        .andExpect(jsonPath("$.message", containsString("Book not found")));
  ...
  void searchBooks_WithFilters_ShouldReturnFilteredBooks() throws Exception {
    ...
    verify(bookService, times(1)).searchBooks("Test", "Author", "Fiction", "Publisher", "1234");
  }

  @Test
  void searchBooks_WithNoFilters_ShouldReturnEmptyList() throws Exception {
    // Arrange
    when(bookService.searchBooks(null, null, null, null, null)).thenReturn(Collections.emptyList());
