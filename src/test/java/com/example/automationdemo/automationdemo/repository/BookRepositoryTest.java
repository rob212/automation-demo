...
/**
 * Repository tests using H2 in-memory database. The @DataJpaTest annotation provides
 * autoconfiguration for JPA tests. It uses an in-memory database by default and disables full
 * autoconfiguration.
 */
@DataJpaTest
@ActiveProfiles("test")
public class BookRepositoryTest {
  ...
  void findByTitleContainingIgnoreCase_WithExistingTitle_ShouldReturnBooks() {
    // Arrange
    bookRepository.save(
        createTestBook("Harry Potter and the Sorcerer's Stone", "J.K. Rowling", "Fantasy"));
    bookRepository.save(
        createTestBook("Harry Potter and the Chamber of Secrets", "J.K. Rowling", "Fantasy"));
  ...
  void findByAuthorContainingIgnoreCase_WithExistingAuthor_ShouldReturnBooks() {
    ...
    assertEquals(2, result.size());
    assertTrue(
        result.stream().allMatch(book -> book.getAuthor().toLowerCase().contains("rowling")));
  }
  ...
  void findByGenreContainingIgnoreCase_WithExistingGenre_ShouldReturnBooks() {
    // Arrange
    bookRepository.save(
        createTestBook("Harry Potter and the Prisoner of Azkaban", "J.K. Rowling", "Fantasy"));
