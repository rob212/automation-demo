...
/**
 * Integration tests for the Book API. These tests use the entire Spring API application stack with
 * a real database.
 */
@SpringBootTest
...
public class BookApiContainerIT {
  ...
  static PostgreSQLContainer<?> postgres =
      new PostgreSQLContainer<>("postgres:${postgresql.version}")
  ...
  void testCreateAndRetrieveBook() throws Exception {
    ...
    BookDTO createdBook =
        objectMapper.readValue(createResult.getResponse().getContentAsString(), BookDTO.class);
  ...
  void testSearchBooks() throws Exception {
    ...
    // Search by genre
    MvcResult genreResult =
        mockMvc
            .perform(get("/api/books/search").param("genre", "Fantasy"))
            .andExpect(status().isOk())
            .andReturn();

    List<BookDTO> genreResults =
        objectMapper.readValue(
            genreResult.getResponse().getContentAsString(), new TypeReference<List<BookDTO>>() {});

    assertEquals(2, genreResults.size());
    assertTrue(genreResults.stream().allMatch(b -> b.getGenre().contains("Fantasy")));

    // Search by author
    MvcResult authorResult =
        mockMvc
            .perform(get("/api/books/search").param("author", "Fantasy Author"))
            .andExpect(status().isOk())
            .andReturn();

    List<BookDTO> authorResults =
        objectMapper.readValue(
            authorResult.getResponse().getContentAsString(), new TypeReference<List<BookDTO>>() {});

    assertEquals(2, authorResults.size());
    assertTrue(authorResults.stream().allMatch(b -> b.getAuthor().equals("Fantasy Author")));

    // Search with multiple criteria
    MvcResult multiResult =
        mockMvc
            .perform(
    ...
    List<BookDTO> multiResults =
        objectMapper.readValue(
            multiResult.getResponse().getContentAsString(), new TypeReference<List<BookDTO>>() {});
