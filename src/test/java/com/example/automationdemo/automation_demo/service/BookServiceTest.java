...
public class BookServiceTest {
    ...
    void searchBooks_ShouldReturnFilteredBooks() {
        ...
        when(bookRepository.findByFilters(
                "Test", "Author", "Fiction", "Publisher", "1234")).thenReturn(Arrays.asList(testBook));

        // Act
        List<BookDTO> result = bookService.searchBooks("Test", "Author", "Fiction", "Publisher", "1234");

        // Assert
        assertEquals(1, result.size());
        assertEquals(testBook.getId(), result.get(0).getId());
        verify(bookRepository, times(1)).findByFilters(
                "Test", "Author", "Fiction", "Publisher", "1234");
    }
}